...
class Plot:
	...
	def plot_entry_exit(self, the_list, marker_type, marker_size, ax1):
		...
		for j in range(0, len(the_list)):
			...
			ax1.plot([entry_time.timestamp()], [entry_price], marker=marker_type, markersize=marker_size, color=color_map[j])
			ax1.plot([exit_time.timestamp()], [exit_price], marker=marker_type, markersize=marker_size, color=color_map[j])
